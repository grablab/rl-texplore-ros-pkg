...
class IL(object):
    def __init__(self, policy, num_states, num_actions, nsteps, nenvs=1, batch_size=40,
        ...
        self.x_ = tf.placeholder(tf.float32, [None, num_states])
        ...
        train_model = policy(observ_placeholder=self.x_, num_actions=num_actions, sess=self.sess)  # this is evaluating common.policies.policy_fn

        # params = self._find_trainable_variables("model/pi")
        params = self._vars("pi")
        for i in params:
            print(i)
